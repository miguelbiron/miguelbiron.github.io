...
var members = [], balances = [], A_mat, inputs_ok = false;
...
function reset_page(){
  ...
  // reset status of inputs_ok
  inputs_ok = false;
  
  // clear inputs missing warning
  document.getElementById("solve-diagnostics").innerHTML = "";
  
  // clear title
  document.getElementById("problem-solved").innerHTML = "";
  
  // clear results table
  var myNode = document.getElementById("results-table");
...
function settle_debts(){
  if(!inputs_ok){
    document.getElementById("solve-diagnostics").innerHTML = "Wait! You still haven't gotten the inputs right";
    return;
  }
  set_const_mat();
  ...
  document.getElementById("problem-solved").innerHTML = "Solution found!";
  print_results(A);
}
